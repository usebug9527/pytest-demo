...
def case_data(request):
    ...
    # # 获取纯文件名（带后缀，如 test_get_token.py）
    ...
    # # 文件名为 test_xxx.py → 提取xxx作为文件标识：test_get_token.py → token
    # if file_name_no_suffix.startswith("test_"):
    #     file_tag = file_name_no_suffix.replace("test_", "")
    # else:
    #     file_tag = file_name_no_suffix
    def_name = request.node.name
    case_data = None
    # 判断当前用例是否在yaml配置清单内，不在则跳过
    if module_name not in CASE_CONFIG_MAP.keys() or def_name not in [case['caseName'] for case in CASE_CONFIG_MAP[module_name]]:
        print(f"module_name: {module_name},CASE_CONFIG_MAP.keys():{CASE_CONFIG_MAP.keys()} def_name: {def_name},CASE_CONFIG_MAP[module_name]:{CASE_CONFIG_MAP[module_name]}")
        pytest.skip(f"跳过用例：文件标识{def_name} ，该用例未配置在test_set.yml")
    for case in CASE_CONFIG_MAP.get(module_name, ""):
        if case.get('caseName', None) == def_name:
